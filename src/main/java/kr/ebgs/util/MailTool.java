...
public class MailTool {
	...
	public void sendConfirmMail(String to,String confirmKey) throws Exception {
		...
		page.append("<div style='padding: 20px 0 10px 0; border-bottom: 5px solid #0059ab; font-weight: bold; font-size: 1.3rem; margin-bottom: 10px;'>"+subject+"</div>");
	...
	@Async
	public void sendFindMail(String to,String confirmKey) throws Exception {
		MimeMessage message = mailSender.createMimeMessage();

		String subject = mailTitle+ " 비밀번호 변경 메일";
		...
		page.append("<div style='width: 500px; min-height: 300e px; padding : 50px; box-shadow: 1px 1px 3px 0 rgba(82, 62, 62, 0.4);"
				+ "font-family: Arial, Helvetica, sans-serif;'>");
		page.append("<div style='padding: 20px 0 10px 0; border-bottom: 5px solid #0059ab; font-weight: bold; font-size: 1.3rem; margin-bottom: 10px;'>"+subject+"</div>");
		page.append("<div style='margin: 30px 0px;font-size: 18px;'>아래의 버튼을 클릭하시면 비밀번호 변경 페이지로 이동됩니다.</div>");
		page.append("<a href='"+GlobalValues.protocol+GlobalValues.domain+"/user/find?key="+confirmKey+"'>"
				+ "<button type='button' style='cursor: pointer;color: #fff;background-color: #007bff;font-size: 1rem;width: 200px;height: 50px;line-height: 50px;font-weight: bold;border-radius: .25rem;'>변경하기</button>"
				+ "</a><br/>");
		page.append("</div>");
		page.append("</div>");
		page.append("</body>");
